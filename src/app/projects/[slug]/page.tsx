...
export default function CaseStudyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const project = projectsData[lang].find((p) => p.slug === slug);
  if (!project) notFound();
      ...
      <main className="pt-[68px]">
        <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.1 }}
            ...
            {imgSrc ? (
              <div className="relative w-full h-[200px] sm:h-[280px] lg:h-[320px]">
                <Image src={imgSrc} alt={project.name} fill className="object-cover" />
              </div>
            ) : (
              <div className="w-full h-[160px] sm:h-[200px]" style={{ background: project.imgGrad }} />
            )}
          </motion.div>

          {/* Sections */}
          <div className="mt-10 sm:mt-12 pb-16 sm:pb-20">
            {cs.sections.map((section, si) => (
              <motion.div key={si} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                ...
                {section.boxes && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-4 sm:mt-5">
                    {section.boxes.map((box, bi) => (
                      <div key={bi} className={`rounded-[14px] p-5 sm:p-7 ${
                ...
                {section.role && (
                  <div className="cs-box-b3 rounded-[14px] p-5 sm:p-7 mt-4">
